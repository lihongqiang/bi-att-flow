...
model_name = "./model/q2p_tf_model_168463/q2p_tf.ckpt-1"
...
with tf.Session(config=config_tf) as sess:
    ...
    ts=[sess.graph.get_tensor_by_name(v.name) for v in tf.all_variables()]
    #for i in range(16,len(ts),2):
        #print(ts[i].eval()==ts[i+1].eval())
    i=1
    j=20
    k=21
    print(ts[i])
