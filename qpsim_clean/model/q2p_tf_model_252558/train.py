...
def main(unused_argv):
    ...
    tmp_model_path = "/tmp/q2p_tf_model_" + str(random.randint(0,1000000))

    if (not os.path.exists(tmp_model_path)):
        os.makedirs(tmp_model_path)
    shutil.copy('q2p_model.py',tmp_model_path)
    shutil.copy('train.py',tmp_model_path)
    ...
    #(Q1, Q2, y_train) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, '/home/wendw/data/DeepQnA/merged_train.tsv'))
    #(Q1, Q2, y_train) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, '/home/wendw/data/MaltaClick/maltaClicks.train.txt.singlelabel'))
    #(Q1_, Q2_, y_test) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, '/home/wendw/data/EQnA/eqna.newLargeValid.txt'))
    #(Q1, Q2, y_train) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, 'fasttest.tsv'))
    #(Q1_, Q2_, y_test) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, 'fasttest.tsv'))
    (Q1, Q2, y_train) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, 'merged_train.tsv'))
    (Q1_, Q2_, y_test) = deepctxt_util.load_raw_data_x1_x2_y(path=os.path.join(data_path, 'eqna_raw_judgments_20151222_norm_valid.tsv'))
    ...
    Q1_test = tokenizer.texts_to_sequences(Q1_, q_maxlen)
    Q2_test = tokenizer.texts_to_sequences(Q2_, p_maxlen)
    print('Done')
    print()

    Y_train = deepctxt_util.to_categorical(y_train, 2).astype(np.float32)
    Y_test = deepctxt_util.to_categorical(y_test, 2).astype(np.float32)
    ...
    Q1_test = deepctxt_util.pad_sequences(Q1_test, maxlen=q_maxlen, padding='post', truncating='post')
    Q2_test = deepctxt_util.pad_sequences(Q2_test, maxlen=p_maxlen, padding='post', truncating='post')
    ...
    with tf.Session(config=config_tf) as sess:
        ...
        summary_writer = tf.summary.FileWriter(model_path + "/{}".format(tmp_model_path.split("_")[-1]), graph=tf.get_default_graph())
        ...
        for epoch in range(1,max_epoch+1):
            ...
            Q1_train = Q1_train[perm]
            Q2_train = Q2_train[perm]
            ...
            for i in range(0, len(Y_train), batch_size):
                _, e, train_accuracy = sess.run(
                              [m_train.train_op, m_train.cost, m_train.accuracy],
                              feed_dict={m_train.input1:Q1_train[i:i+batch_size], 
                                         m_train.input2:Q2_train[i:i+batch_size], 
                                         m_train.targets:Y_train[i:i+batch_size], 
                                         })
                train_accuracies.append(train_accuracy)
                train_losses.append(e)
                cur_step = (epoch-1)*len(Y_train)/batch_size+i/batch_size
                summary = sess.run(tr_merge,feed_dict={p_acc:np.mean(train_accuracies),p_loss:np.mean(train_losses)})
                summary_writer.add_summary(summary,cur_step)

                if cur_step%test_after_steps == 0:
                    test_accuracies = []
                    test_losses = []
                    for j in range(0, len(Y_test), batch_size):
                        e, test_accuracy = sess.run(
                                      [m_test.cost, m_test.accuracy],
                                      feed_dict={m_test.input1:Q1_test[j:j+batch_size], 
                                                 m_test.input2:Q2_test[j:j+batch_size], 
                                                 m_test.targets:Y_test[j:j+batch_size], 
                                                 })

                        test_accuracies.append(test_accuracy)
                        test_losses.append(e)
                    summary = sess.run(te_merge,feed_dict={p_acc:np.mean(test_accuracies),p_loss:np.mean(test_losses)})
                    summary_writer.add_summary(summary,cur_step)
                    print("epoch_%d : train_accuray=%f test_accuray=%f" %(cur_step, np.mean(train_accuracies), np.mean(test_accuracies)))

                progress = float(100*cur_step/(max_epoch*(len(Y_train)/batch_size)))
            ...
            saver.save(sess, os.path.join(tmp_model_path, model_name + ".ckpt"), global_step=i_step)
            #tf.train_write_graph(sess.graph_def, tmp_model_path, model_name + ".pb", as_text=True)
            tf.train.write_graph(sess.graph_def, tmp_model_path, model_name + ".pb", as_text=True)
            i_step += 1
